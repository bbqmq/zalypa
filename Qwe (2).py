# -*-НеA@tDonat-reput}c igame2}ins}ror
import logging
...
-----------------------------------
| Разработчик: Не обработано             |
| Контакты разработчика:          |
|     Telegram: Не известно      |
|     Instagram: None             |
-----------------------------------
|  Скрипт TG BOT: Не определено..    |
-----------------------------------
...
# bot init
bot = Bot(token='5741933292:AAE9ijrVOhGzNF26LaOzUtP2VoAFxNFkaRg')
dp = Dispatcher(bot)

# datebase
connect = sqlite3.connect("qwey.db")
cursor = connect.cursor()
cursor.execute("""CREATE TABLE IF NOT EXISTS banda(
    banda_name STRING,
    banda_reg STRING,
    banda_subs INT,
    banda_like INT,
...
async def posting(message):
    ...
    if user_status == 'Helper_Admin':
       if warn2 > 5:
          ...
          await bot.send_message(1887634547, f"⚠️ | Аккаунт: <a href='tg://user?id={reply_user_id}'>{reply_user_name}</a> был заблокирован ", parse_mode='html')
...
async def start_cmd(message):
    ...
    btn6 = InlineKeyboardButton(text='🆘 Добавить в чат', url='https://t.me/qwegamebot?startgroup')
    register_help = InlineKeyboardButton(text='🆘 Помощь', callback_data='register_help')       
    reg.add(register_help, btn6)

    name = message.from_user.get_mention(as_html=True)
    await bot.send_message(message.chat.id, f'''
<i> <b>❗ • {name}, вы уже зарегистрированы в боте.
🚑 • Если у вас возникла какая то проблема с какой то командой, обратитесь к Хаешка для повторной регистрации [если у вас SPAM BAN, то вы можете обратиться к нему через данного бота с помощью команд (/m [ID] [message]) или через команду (/report)]</b> </i>
...
async def prof_user(message: types.Message):
    ...
    if cursor.fetchone() is None:
       cursor.execute("INSERT INTO banda VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?);",(banda_name, banda_reg, 0, 0, 0, 0, 0, 0, user_id))
       ...
       name = message.from_user.get_mention(as_html=True)
       await bot.send_message(message.chat.id, f'''
<i> <b>📢 • Привет, {name}, я игровой бот.\n🤝 • Тебе как новому пользователю был выдан подарок в размере 10.000$.\n❗ • Для ознакомление с моими командами, введи команду [помощь], или нажмина на кнопку нижи.</b> </i>
    ''', reply_markup=reg, parse_mode='html')
    else:
       status_console = 'off'
       avatarka_start = 'none'
...
#Создать банду
#Банды делал @barmakar короче я
    if message.text.startswith('Банда создать'):
       user_name = cursor.execute("SELECT banda_name from banda where user_id = ?",(message.from_user.id,)).fetchone()
       user_name = user_name[0]
       banda_reg = cursor.execute("SELECT banda_reg from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_reg = str(banda_reg[0])
       banda_name = cursor.execute("SELECT banda_name from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_name = str(banda_name[0])
       chat_id = message.chat.id
       user_id = message.from_user.id
       ...
       if banda_reg in 'on':
          await bot.send_message(message.chat.id, f"😑 | <a href='tg://user?id={user_id}'>{user_name}</a>, у вас уже есть банда!", parse_mode='html')
       else:
          if len(name) <= 20:
             await bot.send_message(message.chat.id, f"😊 | Вы успешно создали банду, чтобы узнать команды банды напиши «команды банды»!", parse_mode='html')
             cursor.execute(f'UPDATE banda SET banda_name = \"{name}\" WHERE user_id = "{user_id}"')
             cursor.execute(f'UPDATE banda SET banda_reg = "on" WHERE user_id = "{user_id}"')
          else: 
             await bot.send_message(message.chat.id, f"{rloser} | <a href='tg://user?id={user_id}'>{user_name}</a>, ник вашей банды не может быть длинее 20 символов!", parse_mode='html')



    if message.text.lower() in ['команды банды', 'Команды банды']:
       msg = message
       user_id = msg.from_user.id
       user_name = cursor.execute("SELECT user_name from users where user_id = ?",(message.from_user.id,)).fetchone()
       user_name = str(user_name[0])
       await bot.send_message(message.chat.id , f"""
<a href='tg://user?id={user_id}'>{user_name}</a>, вот команды банды:
1. Банда создать - создать банду
2. Ограбить банк - ограбить банк
3. Крутая банда - повысить репутацию банды
4. Моя банда - ваша банда
       """, parse_mode='html')
    ...
    if message.text.lower() ==  'ограбить банк':
       ...
       user_name = user_name[0]
       banda_reg = cursor.execute("SELECT banda_reg from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_reg = banda_reg[0]
       user_id = message.from_user.id

       balance = cursor.execute("SELECT balance from users where user_id = ?", (message.from_user.id,)).fetchone()
       balance = int(balance[0])
       banda_videos = cursor.execute("SELECT banda_videos from banda where user_id = ?", (message.from_user.id,)).fetchone()
       banda_videos = int(banda_videos[0])
       period = 300 #300 s = 5m
       get = cursor.execute("SELECT stavka_ad FROM banda WHERE user_id = ?", (message.from_user.id,)).fetchone()
       last_stavka = int(get[0])
       stavkatime = time.time() - float(last_stavka)
       
       rx = random.randint(30000000,12000000000)
       rx2 = '{:,}'.format(rx)

       if banda_reg in 'off':
          await bot.send_message(message.chat.id, f"😢 | <a href='tg://user?id={user_id}'>{user_name}</a>, у вас нет банды, для регистрации напишите: банда создать [ник]!", parse_mode='html')
       else:
          if stavkatime > period:
             await bot.send_message(message.chat.id, f"🤝 | <a href='tg://user?id={user_id}'>{user_name}</a>, вы успешно ограбили банк и получили прибыль {rx2}$", parse_mode='html')
             cursor.execute(f'UPDATE users SET balance = {balance + rx}  WHERE user_id = "{user_id}"')
             cursor.execute(f'UPDATE banda SET stavka_ad = {time.time()} WHERE user_id = "{user_id}"')
             connect.commit()
          else:
             await bot.send_message(message.chat.id, f"⏳ | <a href='tg://user?id={user_id}'>{user_name}</a>, слишком часто ограблять банк тоже вредно, подождите 5 минут!", parse_mode='html')
             
             
             
             
    if message.text.lower() in ["Моя банда", "моя банда"]:
       msg = message
       user_id = msg.from_user.id
       banda_reg = cursor.execute("SELECT banda_reg from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_reg = str(banda_reg[0])
       banda_name = cursor.execute("SELECT banda_name from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_name = str(banda_name[0])
       banda_subs = cursor.execute("SELECT banda_subs from banda where user_id = ?", (message.from_user.id,)).fetchone()
       banda_subs = int(banda_subs[0])
       banda_subs2 = '{:,}'.format(banda_subs)
       banda_like = cursor.execute("SELECT banda_like from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_like = int(banda_like[0])
       banda_videos = cursor.execute("SELECT banda_videos from banda where user_id = ?", (message.from_user.id,)).fetchone()
       banda_videos = int(banda_videos[0])
       banda_videos2 = '{:,}'.format(banda_videos)
       user_name = cursor.execute("SELECT user_name from users where user_id = ?",(message.from_user.id,)).fetchone()
       user_name = str(user_name[0])
       if banda_reg in ['off']:
          await bot.send_message(message.chat.id, f"😢 | <a href='tg://user?id={user_id}'>{user_name}</a>, у вас нету банды, для регистрации напишите: банда создать [ник]!", parse_mode='html')
       else:
          photo = open('banda.jpg', 'rb')
          await bot.send_photo(message.chat.id, photo, f"🚨 | <a href='tg://user?id={user_id}'>{user_name}</a>, информация о вашей банде:\nНик: «{banda_name}»\n👤 | Бандитов: {banda_subs2}\n❤️ | Репутация: {banda_like}", parse_mode='html')
             
             
             
    if message.text.lower() ==  'найти бандитов':
       user_name = cursor.execute("SELECT user_name from users where user_id = ?",(message.from_user.id,)).fetchone()
       user_name = user_name[0]
       banda_reg = cursor.execute("SELECT banda_reg from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_reg = banda_reg[0]
       ...
       get = cursor.execute("SELECT stavka_banda FROM banda WHERE user_id = ?", (message.from_user.id,)).fetchone()
       last_stavka = int(get[0])
       stavkatime = time.time() - float(last_stavka)
       ...
       if banda_reg in 'off':
          ...
          if stavkatime > period:
             await bot.send_message(message.chat.id, f"🚨 | <a href='tg://user?id={user_id}'>{user_name}</a>, вы успешно нашли {rx2} новых бандитов, ограбляйте вместе!", parse_mode='html')
             cursor.execute(f'UPDATE banda SET banda_subs = {banda_subs + rx}  WHERE user_id = "{user_id}"')
             cursor.execute(f'UPDATE banda SET banda_videos = {banda_videos + 1}  WHERE user_id = "{user_id}"')
             cursor.execute(f'UPDATE banda SET stavka_banda = {time.time()} WHERE user_id = "{user_id}"')
             connect.commit()
          else:
             await bot.send_message(message.chat.id, f"⏳ | <a href='tg://user?id={user_id}'>{user_name}</a>, слишком часто искать бандитов тоже вредно, подождите 10 минут!", parse_mode='html') 



    if message.text.lower() ==  'крутая банда':
       msg = message
       reply_user_id = msg.reply_to_message.from_user.id
       user_name = cursor.execute("SELECT user_name from users where user_id = ?",(message.from_user.id,)).fetchone()
       user_name = user_name[0]
       banda_name = cursor.execute("SELECT banda_name from banda where user_id = ?",(message.from_user.id,)).fetchone()
       banda_name = banda_name[0]
       user_id = message.from_user.id
       ...
       banda_like = cursor.execute("SELECT banda_like from banda where user_id = ?", (message.from_user.id,)).fetchone()
       banda_like = int(banda_like[0])
       reply_banda_reg = cursor.execute("SELECT banda_reg from banda where user_id = ?",(message.reply_to_message.from_user.id,)).fetchone()
       reply_banda_reg = str(reply_banda_reg[0])
       reply_user_name = cursor.execute(f"SELECT user_name from users where user_id = {reply_user_id}").fetchone()
       reply_user_name = str(reply_user_name[0])
       period = 300 
       get = cursor.execute("SELECT stavka_like FROM banda WHERE user_id = ?", (message.from_user.id,)).fetchone()
       last_stavka = int(get[0])
       stavkatime = time.time() - float(last_stavka)

       if reply_banda_reg in 'off':
          await bot.send_message(message.chat.id, f"⛔ | <a href='tg://user?id={user_id}'>{user_name}</a>, у пользователя нету банды!", parse_mode='html')
       else:
          
          if stavkatime > period:
             await bot.send_message(message.chat.id, f"❤️ | <a href='tg://user?id={user_id}'>{user_name}</a>, вы успешно повысили репутацию банды игрока <a href='tg://user?id={reply_user_id}'>{reply_user_name}</a>!", parse_mode='html')
             cursor.execute(f'UPDATE banda SET banda_like = {banda_like + 1}  WHERE user_id = "{reply_user_id}"')
             cursor.execute(f'UPDATE banda SET stavka_like = {time.time()} WHERE user_id = "{user_id}"')
             connect.commit()
          else:
             await bot.send_message(message.chat.id, f"⏳ | <a href='tg://user?id={user_id}'>{user_name}</a>, слишком часто повышать репутацию тоже вредно, подождите 5 минут!", parse_mode='html') 
...
⛔️Прошу вас соблюдать правила отправки репорта
    ...
    if message.text.lower() in ["Б", "б"]:
       ...
       hel2 = InlineKeyboardMarkup(row_width=2)
       osnman = InlineKeyboardButton(text='❗ Записаться в модераторы', callback_data='osnman')
       osn = InlineKeyboardButton(text='👁 Другое', callback_data='osn')
       hel2.add(osnman, osn)
       await bot.send_message(message.chat.id, f'''
<b>🌵 • <a href='tg://user?id={user_id}'>{user_name}</a>, твой балансик!
...
🤑 • Всего денег: [{balance+bank}$]

👇 • Тыкай на кнопки ниже и управляй!</b>

    ''', reply_markup=hel2, parse_mode='html')
#Семьи
...
###########################################АДМИН КОМАНДЫ###########################################
    if message.text.startswith("Поделить"):
       ...
       if user_status[0] == 'Admin':
          ...
          await message.reply(f"<a href='tg://user?id={user_id}'>{user_name}</a> ,Вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь к разработчику @hioka_official ⚠️", parse_mode='html')
...
📊 @wiol_full - Игровой канал
💬 @wiol_chat1 - Игровой чат
🧑‍💻 @hioka_official - Разработчик
          ...
          if balance >= summ:
             if summ > 0:
                ...
                if int(rx) in range(10201, 10210):
                   ...
                   i2 = int(i)
                   i3 = '{:,}'.format(i2)
                   await bot.send_message(1887634547, f"🤵‍♂️ | Игрок: <a href='tg://user?id={user_id}'>{user_name}</a>\n🤵‍♀️ | Игра: Казино\n🎟 | Ставка: {summ2}$\n🧾 | Выигрыш: {i3}$ [10X]", parse_mode='html')
...
ℹ️ | Чтобы купить Donate-coins, обратитесь к владельцу бота @hioka_official
...
async def craft_resurs3(callback: types.CallbackQuery):
   ...
   if user_status in ['Admin', 'Helper_Admin', 'Owner']:
      ...
      await callback.message.answer( f"<a href='tg://user?id={user_id}'>{user_name}</a>, вы не являетесь администратором бота 👮. Для получение прав администратора обратитесь к разработчику @hioka_official ⚠️", parse_mode='html')
...
🛒 Для покупки администраторских прав , обратитесь к Владельцу бота - @hioka_official
    """,  parse_mode='html' )
...
🧮 • .cl 2+2 - данная команда позволяет считать любые математические примеры - ТЕХ РАБОТЫ
...
👨‍💼 • Бизнес - Показывает все команды бизнеса
...
@dp.callback_query_handler(text='osnman')
async def help(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    user_name = cursor.execute("SELECT user_name from users where user_id = ?",(callback.from_user.id,)).fetchone()
    user_name = str(user_name[0])
    await callback.message.answer(f'''
<b>👌 • <a href='tg://user?id={user_id}'>{user_name}</a>, чтобы записаться в модераторы напишите <a href="https://t.me/qwetopma">ему</a>!</b>''', parse_mode='html')

@dp.callback_query_handler(text='osn')
async def help(callback: types.CallbackQuery):
    user_id = callback.from_user.id
    user_name = cursor.execute("SELECT user_name from users where user_id = ?",(callback.from_user.id,)).fetchone()
    user_name = str(user_name[0])
    # МНЕ ЛЕНЬ
    await callback.message.answer(f'''
<b>🍓 • <a href='tg://user?id={user_id}'>{user_name}</a>, другое:
	
🪙 • Donate-coins: 0
🃏 • Всего сыграно игр: в разработке
❗️ • Привилегия: в разработке
🔝 • Репутация: 0</b>
